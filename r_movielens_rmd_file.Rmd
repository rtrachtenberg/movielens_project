...
Now, since there are so many distinct genre groups in the dataset (due to the possibility of combinations of genres) let's see if we can split and visualize the top 10 genres, let's clean/organize the genre data and plot the top 10 genres:
...
It looks drama is our top genre, with comedy close behind in terms of number of ratings. However, note that one movie may occupy two to several genres.
...
Let's start by running code provided by instructors to get an idea of what a simple model would yield if we just considered movie effect in addition to overall rating mean:
...
Now let's see if we can improve our model by adding in regularization:
...
Let's try an ensemble method, using random forest and a subset of the training data due to data processing time and capabilities:
...
This is an interesting exercise in that it gives an RMSE similar to that of our basic movie effect model. Instead of running a more comprehensive random forest model (that would likely take multiple hours to run) with a larger portion or the full dataset.
